...
async function callService({ serviceName, baseUrl, apiToken }, method, path, data = null, options = {}) {
  if (!baseUrl) {
    throw new Error(`${serviceName} base URL not configured`);
  }

  if (!apiToken) {
    throw new Error(`${serviceName} API token not configured`);
  }
  ...
  console.log(`[Chatbot Service] Request headers:`, JSON.stringify(config.headers, null, 2));
  ...
  console.log(`[Chatbot Service] Calling ${serviceName}: ${method.toUpperCase()} ${url}`);
  
  try {
    const response = await axios(config);
    return response;
  } catch (error) {
    console.error(`[Chatbot Service] ${serviceName} call failed:`, error.message);
